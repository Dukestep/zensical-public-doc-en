// "Collapse all / Expand all" control for the "Data" and "Usage and
// tutorials" sidebar trees — the only two deep enough to need it.
// Applied via a `data-nav-force` attribute (see matching !important
// rules in extra.css) rather than the checkbox alone, since a section
// already toggled by hand doesn't respond to a plain checkbox change.
(function () {
  "use strict";

  var SCOPED_SECTIONS = ["Data", "Usage and tutorials"];
  ...
  function init() {
    ...
    function setLabel(expanded) {
      link.textContent = expanded ? "Collapse all" : "Expand all";
      link.setAttribute("aria-label", link.textContent + " navigation sections");
