...
- [x] Exact matches: `=` match features where an attribute has exact value;</br>
- [x] Partial string or keywords: `LIKE` with `%` wildcard;</br>
- [x] Comparison operators: `>`, `<`, `>=`, `<=` filter numeric or temporal values;</br>
- [x] Logical operators:`AND`, `OR`, `NOT` compare conditions;</br>
- [x] Spatial predicates: `INTERCEPTS`, `DISJOINT` filter based on geometry.</br>
